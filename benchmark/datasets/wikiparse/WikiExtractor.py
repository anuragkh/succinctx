...
class WikiCleanerThread(threading.Thread):
    ...
    def __init__(self, queue, outputdir, maxfilesize, prefix, compress, output_format, output_clean):
        ...
        self._output_clean = output_clean
...
def process_data(inputdump, outputdir, maxfilesize, compress, outformat, outclean):
    ...
    for _ in range(multiprocessing.cpu_count()):
        cleaner = WikiCleanerThread(queue, outputdir, maxfilesize, prefix, compress, outformat, outclean)
...
def main():
    ...
    parser.add_argument("-k", "--clean", default=False, action="store_const", const=True, help="clean the text up for JSON output")
   ...
   # Minimum size of output files
    ...
    if args.wikidump.lower().endswith("bz2"):
        with bz2.BZ2File(args.wikidump, 'r') as inputdump:
            process_data(inputdump, args.outputdir, file_size, args.compress, args.format.lower(), args.clean)
    else:
